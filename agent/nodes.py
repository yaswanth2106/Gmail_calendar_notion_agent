"""
LangGraph Nodes — individual steps in the agent workflow.

Nodes:
  1. email_reader   → Fetches emails from Gmail
  2. planner        → LLM analyses each email and decides action
  3. executor       → Runs the decided actions via ReAct engine
  4. summarizer     → Produces a final summary of all actions taken
"""
...
class AgentState(TypedDict):
    """
    Typed dict serving as LangGraph state.
    """
    emails: list[dict]
    decisions: list[dict]
    results: list[dict]
...
class PlannerNode:
    """Analyses each email and decides the action."""
...
class ExecutorNode:
    """Executes the planned actions using the ReAct engine and MCP tools."""
...
class SummarizerNode:
    """Produces a human-readable summary of all actions taken."""
